...
const EPSPrediction: React.FC<EPSPredictionProps> = ({stockId}) => {
  ...
  const [chartData, setChartData] = useState<{ year: number; reportedEPS: number; estimatedEPS: number }[]>([]);
                ...
                <Card className='shadow-lg hover:shadow-xl transition-shadow duration-300'>
                  ...
                  <CardContent>
                    {period.Actuals?.Actual?.[0] && ( 
                      <div>
                        <p className=''>
                          Reported EPS: <span className='font-semibold'>{period.Actuals.Actual[0].Reported}</span> {period.Actuals.Actual[0].CurrencyCode}
                        </p>
                    ...
                    {period.Estimates?.Estimate?.[0] && ( 
                      <div>
                        <p className=''>Estimate Mean EPS: <span className='font-semibold'>{period.Estimates.Estimate[0].Mean}</span></p>
