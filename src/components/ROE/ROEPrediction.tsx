...
const ROEPrediction: React.FC<ROEPredictionProps> = ({stockId}) => {
  ...
  const [chartData, setChartData] = useState<{ year: number; reportedROE: number; estimatedROE: number }[]>([]);
                ...
                <Card className='shadow-lg hover:shadow-xl transition-shadow duration-300'>
                  ...
                  <CardContent>
                    {period.Actuals?.Actual?.[0] && ( 
                      <div>
                        <p className=''>
                          Reported ROE: <span className='font-semibold'>{period.Actuals.Actual[0].Reported}</span> {period.Actuals.Actual[0].CurrencyCode}
                        </p>
                    ...
                    {period.Estimates?.Estimate?.[0] && ( 
                      <div>
                        <p className=''>Estimate Mean ROE: <span className='font-semibold'>{period.Estimates.Estimate[0].Mean}</span></p>
                        <p>High: <span className='bg-green-600/50 px-2 rounded-md'>{period.Estimates.Estimate[0].High}</span></p>
                        <p>Low: <span className='bg-red-600/50 px-2 rounded-md'>{period.Estimates.Estimate[0].Low}</span></p>
